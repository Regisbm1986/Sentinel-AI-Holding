...
def test_autonomous_status_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("backend.agents.task_queue.AGENT_TASKS_DIR", tasks_dir)
...
def test_telemetry_endpoint_returns_logs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: __import__(
            "backend.telemetry.execution_telemetry",
...
def test_autonomous_status_and_run_cycle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("backend.api.main.PROJECT_ROOT", project_root)
    monkeypatch.setattr("backend.agents.task_queue.AGENT_TASKS_DIR", tasks_dir)
    monkeypatch.setattr("backend.agents.task_history.AGENT_TASKS_DIR", tasks_dir)
    monkeypatch.setattr("backend.agents.workflow_state_manager.AGENT_TASKS_DIR", tasks_dir)
    monkeypatch.setattr("backend.agents.remote_worker_manager.AGENT_TASKS_DIR", tasks_dir)
...
def test_telemetry_returns_list(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("backend.api.main.ExecutionTelemetry", lambda *args, **kwargs: __import__("backend.telemetry.execution_telemetry", fromlist=["ExecutionTelemetry"]).ExecutionTelemetry(log_path=telemetry_path))
