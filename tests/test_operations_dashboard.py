from backend.agents.worker_heartbeat import WorkerHeartbeat
...
def test_dashboard_snapshot_uses_real_project_data(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.agents.remote_worker_manager.RemoteWorkerManager.get_workers",
...
def test_dashboard_loaders_return_expected_sections(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("backend.agents.task_queue.AGENT_TASKS_DIR", tasks_dir)
    monkeypatch.setattr("backend.agents.remote_worker_manager.RemoteWorkerManager.get_workers", lambda self: [])
